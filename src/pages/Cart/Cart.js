...
const Cart = () => {
      ...
      return (
            <div className="container h-auto mx-auto mb-11 p-4">
                  ...
                  {cartItems.length === 0 ? (
                        <p className='text-red-600 text-center'>No items in cart.</p>
                  ) : (
                        <table className="w-full border-collapse">
                              <thead className='text-white' style={{ backgroundImage: "linear-gradient(to right, black ,#009498 )" }}>
                                    <tr>
                                    ...
                                    {cartItems.map((item, index) => (
                                          <tr key={index} className="hover:bg-gray-100 bg-gray-50">
                                                <td className="border-b border-gray-300 px-4 py-2">
                                                      <img src={item.image} alt={item.name} className="w-16 h-16 rounded mx-auto" />
                                                </td>
