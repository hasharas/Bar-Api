...
import latestService from '../../services/LatestService'; // Import the service

const Latest = () => {
      const [cockTail, setCockTail] = useState([]);
      const [currentPage, setCurrentPage] = useState(1); //page track
      const itemsPerPage = 8; // 8 per page
      const navigate = useNavigate();

      useEffect(() => {
            const fetchCockTail = async () => {
                  try {
                        const data = await latestService.fetchLatestDrinks('Cocktail'); // Use service
      ...
      return (
            ...
            >
                  <hr className="mb-3" />
                  <p className="text-3xl mb-5 font-sans pl-2 font-bold text-[#009498]">
                        Latest Drinks
                  </p>
